...
import EventList from "../EventList"; // Unsure why this value is never read / used
...
describe("<Event /> component", () => {
  ...
  test("test if event wrapping div renders", () => {
    expect(EventWrapper.find(".event")).toHaveLength(1);
  });

  test("test if event wrapping div displays an event__Overview", () => {
    expect(EventWrapper.find(".event").children()).toHaveLength(1);
  });

  test("test if event__Overview renders", () => {
    expect(EventWrapper.find(".event__Overview")).toHaveLength(1);
  });

  test("test if event__Overview children render", () => {
    expect(EventWrapper.find(".event__Overview").children()).toHaveLength(3);
  });

  test("test if event__Details children render", () => {
  ...
  test("test if show and/or hide details button renders", () => {
    expect(EventWrapper.find(".event__Overview button")).toHaveLength(1);
  });

  test("if / when click on button the details display", () => {
    EventWrapper.setState({
      showDetails: false,
    });
  ...
  test("set the mock event data as a state", () => {
    EventWrapper.setState({
      event: {
